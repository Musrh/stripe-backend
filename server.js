...
app.post("/create-stripe-session", async (req,res)=>{
  ...
  try {
    const line_items = items.map(i=>({
      price_data:{ currency:"eur", product_data:{ name:i.nom, images:[i.image || "/placeholder.png"] }, unit_amount: i.prix*100 },
      quantity: i.quantity
    }));
...
app.post("/capture-paypal-order", async (req,res)=>{
  ...
  try{
    ...
    await db.collection("commandes").add({ paypalOrderId: orderId, email:user.email, items, adresseLivraison, montant, devise, statut:"payé", date: admin.firestore.FieldValue.serverTimestamp() });
    res.json({ capture });
